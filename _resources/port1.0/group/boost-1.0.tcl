...
# RJVB : full depspec with type on the devport
default boost_cache_devport_dep   ""
default boost_cache_devport_type  ""
...
proc boost::configure_build {} {
    ...
    global boost_cache_devport_dep boost_cache_devport_type
    ...
    if { ${boost_cache_devport_dep} ne ""} {
        depends_${boost_cache_devport_type}-delete ${boost_cache_devport_dep}
    }
    set boost_cache_depends       [option boost.depends_type]
    set boost_cache_version_nodot [boost::version_nodot]
    set boost_cache_devport_dep   port:boost[boost::version_nodot]-dev

    depends_[option boost.depends_type]-append port:boost[boost::version_nodot]
    if {[option boost.depends_type] ne "run"} {
        # With the macstrop implementation, the boost headers are in boostXYZ-dev
        # so we need to add a depends_build on that devport, unless we're
        # declaring a depends_run.
        depends_build-append      port:boost[boost::version_nodot]-dev
        set boost_cache_devport_type build
    } else {
        # let depends_run port:boostXYZ behave like the port wasn't split
        depends_run-append        port:boost[boost::version_nodot]-dev
        set boost_cache_devport_type run
    }
    if { [option boost.require_numpy] } {
