...
# around until we have more use cases...
   post-extract {
       ...
       if {![file exists ${worksrcpath}] && \
           ...
           set tarroot ${gitlab.project}-${git.branch}
           ui_debug "checking for \"${workpath}/${gitlab.author}-${gitlab.project}-*\" and \"${workpath}/${tarroot}-*\""
           ...
           } elseif {[file exists [glob -nocomplain ${workpath}/${tarroot}-*]] && \
               [file isdirectory [glob -nocomplain ${workpath}/${tarroot}-*]]} {
               move [glob ${workpath}/${tarroot}-*] ${workpath}/${distname}
           } else {
               # tarball is not "${gitlab.author}-${gitlab.project}-*"
               ui_error "\n\ngitlab PortGroup: Error: \${worksrcpath} does not exist after extracting distfiles. \
               This might indicate that the author or project is different than set in the Portfile due to a rename at Gitlab. \
               Please examine the extracted directory in ${workpath} and try to correct the Portfile by either changing \
               the author or project or adding the worksrcdir option with the correct directory name.\n"
