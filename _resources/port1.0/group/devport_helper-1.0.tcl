...
namespace eval devport_helper {
    ...
    if {[info exists ::argv]} {
        ...
        if {!([file exists ${currentportgroupdir}/devport_db.tcl]
            ...
            ui_debug "devport_db doesn't exist or is outdated"
            set updatedb 1
        }
    }
    ...
    proc generate_db {} {
        ...
        if {![catch {set fp [open ${devport_helper::currentportgroupdir}/devport_db.tcl "w"]} err]} {
            ...
            foreach dp ${devports} {
                ...
                set mainport [string range ${dp} 0 [string last "-dev" ${dp}]-1]
                # we might want to impose that mainport and devport share the same portdir
                # but that probably makes us lose our benefit if users install ports from, say, an
                # otherwise shadowed LinuxPorts on Darwin.
                # if {[exec port -q dir ${mainport}] eq [exec port -q dir ${dp}]}
                # gst-dev is (most likely) an upstream port, though.
                if {${dp} ne "gst-dev" && ${dp} ne "mp4v2-dev"} {
                    puts ${fp} "  ${mainport}\t${dp} \\"
                }
            }
            puts ${fp} "\]"
